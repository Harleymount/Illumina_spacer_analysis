...
ggplot(hist_data, aes(x=loss_length))+geom_histogram(binwidth=1,color='white',fill='gray0')+Cleanup+labs(x='Size of Continuous Array Loss', y='Frequency')+ggtitle('Array Size Loss Frequency Among Reads') + theme(plot.title = element_text(hjust = 0.5)) 
dev.off()
